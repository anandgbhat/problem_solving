...
func main() {
  ...
  fmt.Printf("shortest super sequence of %s and %s = %s\n",
    str1, str2, shortestSuperString(str1, str2, lcs))
}

func shortestSuperString(str1, str2, lcs string) string {
  i,j,k := 0,0,0
  var output []byte
  str1Byte := []byte(str1)
  str2Byte := []byte(str2)
  ...
  for {
    ...
    // traverse str1Byte till we hit lcs[0]
    // All non-matching characters go into output
    for i < len(str1Byte) && str1Byte[i] != lcsByte[k] {
      output = append(output, str1Byte[i])
      i++
    }
    // traverse str2Byte till we hit lcs[0]
    // All non-matching characters go into output
    for j < len(str2Byte) && str2Byte[j] != lcsByte[k] {
      output = append(output, str2Byte[j])
      j++
    }
    // Move to next character in LCS
    if str1Byte[i] == lcsByte[k] && str2Byte[j] == lcsByte[k] {
      output = append(output, str1Byte[i])
      i++
      j++
      k++
